...
- For a one-off artifact or result, use the registered `gpt-image-2` service in the `runapi` CLI. If the installed command catalog does not list it, stop and report the missing service instead of inventing a command.
...
runapi gpt-image-2 <operation> --help
curl --fail --location https://runapi.ai/docs/api/gpt-image-2/<operation>.md --output contract.md
```
...
```shell
runapi gpt-image-2 <operation> --async --input-file request.json > task.json
...
```shell
runapi wait "$task_id" --service gpt-image-2 --action <operation> > result.json
```
